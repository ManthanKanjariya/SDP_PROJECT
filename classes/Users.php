...
Class Users extends DBConnection {
	...
	function registration(){
		...
		$data = "";
		$check = $this->conn->query("SELECT * FROM `tutor_list` where email = '{$email}' ".($id > 0 ? " and id!='{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `tutor_list` set {$data} ";
		}else{
			$sql = "UPDATE `tutor_list` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			if(!empty($_FILES['img']['tmp_name'])){
				...
				$upload =imagepng($temp,base_app.$fname);
				if($upload){
					$this->conn->query("UPDATE `tutor_list` set `avatar` = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$uid}'");
				}
				imagedestroy($temp);
			}
			if(!empty($uid) && $this->settings->userdata('login_type') != 1){
				$user = $this->conn->query("SELECT * FROM `tutor_list` where id = '{$uid}' ");
				if($user->num_rows > 0){
			...
			if(!empty($id) && !empty($data)){
				$this->conn->query("DELETE FROM `tutor_meta` where tutor_id = '{$uid}' ");
			}
			if(!empty($data)){
				$sql2 = "INSERT INTO `tutor_meta` (`tutor_id`, `meta_field`, `meta_value`) VALUES {$data}";
				...
				if(!$save2){
					...
					$resp['sql'] = $sql2;
					if(empty($id)){
						$this->conn->query("DELETE FROM `tutor_list` where id = '{$uid}'");
					}
				}
			}
		}else{
